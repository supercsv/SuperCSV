...
// ValidateDatetimeTZ validates ISO 8601 datetime WITH required timezone using position-based checks.
...
func ValidateDatetimeTZ(b []byte) bool {
	n := len(b)
	...
	if pos == n {
		return false // datetimetz requires timezone - reject if missing
	}
...
func DatetimeTZError() string {
	return "invalid datetimetz format (expected ISO 8601 with required timezone)"
}
